---
title: "Replication Has Limited Power to Detect Bad Science"
...
*Replication seems a sensible way to assess whether a scientific result is right. The intuition is clear: if a a result is right, you should get a similar answer when repeating the work; if it it's wrong, your answer should be quite different. Statisticians have devised numerous statistical tests for deciding whether a replication passes or fails, thus validating of refuting the original result. Using simulation, I simulate many of these tests across a range of conditions. For exact replications, simple significance testing works great. When replications diverge from the original studies, no test works well. Much of the replication literature focusses, perhaps unwittingly, on methods for divergent conditions without mentioning the limited power of replication in this case.*
...
By default $n$ varies from 10 to 5120 in 10 steps separated by powers of 2 (i.e., $\{10,20,40,...,5120\}$) and $d_{pop}$ varies from 0 to 1 in 11 steps of 0.1. In total, this comes to $1.1\times10^6$ ($10^4\times10\times11$) study instances. When analyzing results, I interpolate to get intermediate values; this is especially important for $n$ since the grid of values is so widely spaced.

To generate pairwise replications, I consider all (ordered) pairs of study sets. For each pair, the software permutes the instances of each study, then combines the instances row-by-row to get *pairwise replication instances*. In total, this comes to $1.21\times10^8$ ($10^4\times10\times11\times10\times11$) pairwise replication instances. It is often convenient to think of the first study of the pair as the *original* and the second as the *replication*, but this distinction is not baked into the software. 

A *pairwise replication set* is the ensemble of pairwise replication instances for a given pair of study sets. Four variables parameterize each pairwise replication set: $n1$, $n2$, $d1{pop}$, $d2_{pop}$. These are, naturally enough, the sample and population effect sizes for the two study sets.
...
The code implements 28 measures. This includes all replication rules I found in the literature or blogosphere, plus ones I added to satisfy my sense of completeness, and a few I needed for technical reasons.  The [Measures section below](#measures) has them all, but here a few to give you a flavor.

- *sig1* (resp. *sig2*) - the first (resp. second) study of the pair has a significant p-value
- *d1.c2* (resp. *d2.c1*) - $d1_{sdz}$ (resp. $d2_{sdz}$), the standardized observed effect size (aka Cohen's d) of the first (resp. second) study is in the confidence interval of the second (resp. first) study
- *d1.p2* (resp. *d2.p1*) - $d1_{sdz}$ (resp. $d2_{sdz}$) is in the prediction interval of the second (resp. first) study
- *sdir* - the observed effects sizes for the two studies are in the same direction, i.e., have the same sign

The next step sumarizes the results by counting the number of true results for each measure in the replication set. I compute counts for the measures individually and a few combinations of interest.  The combinations are

- *sig1* - instances that satisfy the *sig1* measure and each measure, since most authors assume that the original study is significant
- *baseline* - instances that also satisfy *sdir*, because most authors assume that the observed effect size are in the same direction
...
The final step is to convert pass rates into true positive, false positive, true negative, and false negative rates. This requires a definition of *true* and *false* instances, in other words, an explicit statement as to which pairwise replication instances represent replications that should succeed vs. ones that should fail.
...
I'm not the right person to define really good correctness criteria for replication (or any other substantial statistics problem), but since I need such criteria to proceed with this post, here goes.

Replication researchers seem to study two aspects of correctness.  

1. The most basic concern is that the original study is a false positive. For the simple studies in my simulation, this means the population effect size for the original study is non-zero, but the study's p-value is significant nonetheless.

2. The other concern is that the observed effect sizes are inconsistent with each other. This might mean that the population effect sizes for the two studies are different, or that the one result is an outlier.

The first concern seems totally sensible, or more accurately, as sensible as any other use of the p-value, null hypothesis testing framework. The second concern baffles me: if the two studies are sampling different populations or one is an outlier, why does this invalidate the first study?

I'm tempted to ignore the second concern, but most of the proposed replication methods address it.  Sadly, I'm stuck with it for the time being.

Here are the precise technical definitions I use in the code.

1. *non-zero* - an instance is *true* if $d1_{pop}$, the population effect size for the first study, is non-zero
2. *same-effect* - an instance is *true* if $d1_{pop}=d2_{pop}$, i.e., the two population effect sizes are equal

I also consider a modest extension that allows the effect sizes to differ from 0 or each other by a specified *tolerance* $\epsilon$:

1. *non-zero* is *true* if $d1_{pop}\ge\epsilon$
2. *same-effect* is *true* if $abs(d1_{pop}-d2_{pop})\ge\epsilon$

Note that these criteria depend only on the population effect size.

### Measures

The table below lists the 28 measures implemented by the software. Each measure applies to a pairwise replication instance (although some only depend on one study of the pair). Measures use the following notation.

- *s1* and *s2* refer to the first and second study of the pair; *sm* means the fixed-effect meta-analysis of the two studies
- $d1_{sdz}$, $d2_{sdz}$, $dm_{sdz}$ mean the standardized observed effect size (aka Cohen’s $d$) of *s1*, *s2*, *sm*
- *c1*, *c2*, *cm* are confidence intervals of *s1*, *s2*, *sm*; my implementation of confidence intervals is based on [Uri's Simonsohn's code](http://urisohn.com/sohn_files/BlogAppendix/Colada20.ConfidenceIntervalsForD.R) supporting his post [We cannot afford to study effect size in the lab](http://datacolada.org/20)
- *p1*, *p2* are prediction intervals of *s1*, *s2*; the method is from a [paper by Jeffrey Spence and David Stanley](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5028066/) with code adapted from the [Stanley's predictionInterval package](https://cran.r-project.org/web/packages/predictionInterval)
- *scp1*, *scp2* are small telescopes thresholds of *s1*, *s2*; the method is from Uri's [Small Telescopes paper](http://datacolada.org/wp-content/uploads/2015/05/Small-Telescopes-Published.pdf) and post [Accepting the Null: Where to Draw the Line?](http://datacolada.org/42) with code adapted from [Uri's code](http://urisohn.com/sohn_files/BlogAppendix/Colada42/%5b42%5d%20-%20Accepting%20the%20Null%20-%20R%20-%20Reproducibility%20project%20small%20telescope%20vs%20bayesian%202015%2010%2026.R) supporting that post
...
- *.* combines two statistics to yield a complete measure. For example, *d1.c2* means that $d1_{sdz}$ is contained in *c2*; *c1.c2* means *c1* and *c2* overlap; *d1.scp2* means $d1_{sdz} \ge scp2$

Please see the Technical Note document for a more complete explanation of the measures.

| measure |     meaning     | 
| ------------- | -------------------------------------------- |
| sig1 | p-value of *s1* significant | 
| sig2 | p-value of *s2* significant | 
| sigm | p-value of *sm* significant | 
| sdir | $d1_{sdz}$ and $d2_{sdz}$ are in same direction, i.e., have same sign | 
| d1.c2 | $d1_{sdz}$ contained in confidence interval of *s2* | 
| d2.c1 | $d2_{sdz}$ contained in confidence interval of *s1* | 
| d1.cm | $d1_{sdz}$ contained in confidence interval of *sm* | 
| d2.cm | $d2_{sdz}$ contained in confidence interval of *sm* | 
| dm.c1 | $dm_{sdz}$ contained in confidence interval of *s1* | 
| dm.c2 | $dm_{sdz}$ contained in confidence interval of *s2* | 
| c1.c2 | confidence intervals of *s1* and *s2* overlap | 
| c1.cm | confidence intervals of *s1* and *sm* overlap |
| c2.cm | confidence intervals of *s2* and *sm* overlap | 
| d1.p2 | $d1_{sdz}$ contained in prediction interval of *s2* | 
| d2.p1 | $d2_{sdz}$ contained in prediction interval of *s1* | 
| dm.p1 | $dm_{sdz}$ contained in prediction interval of *s1* | 
| dm.p2 | $dm_{sdz}$ contained in prediction interval of *s2* | 
| p1.p2 | prediction intervals of *s1* and *s2* overlap | 
| d1.scp2 | $d1_{sdz}$ exceeds small telescope threshold for *s2* | 
| d2.scp1 | $d2_{sdz}$ exceeds small telescope threshold for *s1*; note that Uri's published small telescope method is this one and requires that *sig2* also hold | 
| dm.scp2 | $dm_{sdz}$ exceeds small telescope threshold for *s2* | 
| dm.scp1 | $dm_{sdz}$ exceeds small telescope threshold for *s1* | 
| d1.scpd2 | $d1_{sdz}$ exceeds threshold from my extension to small telescope method for *s2* | 
| d2.scpd1 | $d2_{sdz}$ exceeds threshold from my extension to small telescope method for *s1* | 
| dm.scpd2 | $dm_{sdz}$ exceeds threshold from my extension to small telescope method for *s2* | 
| dm.scpd1 | $dm_{sdz}$ exceeds threshold from my extension to small telescope method for *s1* | 
| big1 | $d1_{sdz} \ge d2_{sdz}$ | 
| big2 | $d2_{sdz} \ge d1_{sdz}$ | 

TODO: explain which measures we use in the analysis, for plots and heats. explain that we look separately at scopes since they assume *sig2*
