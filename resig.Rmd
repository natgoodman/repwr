...
The program varies $n$ from 20 to 500 and $d_{pop}$ from 0 to 1; in total, I simulate 14,641 conditions each with $10^4$ simulated replications, for a grand total of almost 150 million simulated instances.  When analyzing results, I interpolate to get values not simulated directly.

An important limitation is that I assume all population effect sizes to be equally likely within the range studied. I don't consider *publication bias* which may make smaller effect sizes more likely, or any prior knowledge of expected effect sizes.
...
- a *false positive replication* is one where the original study is invalid, yet the replica passes the test: $d1_{pop}=0$, $pval2<=0.05$ and the effect sizes are in the same direction
- a *false negative replication* is one where the original study is valid, yet the replica fails the test: $d1_{pop}\ne0$ and  $pval2>0.05$ or the effect sizes are in opposite directions
...
I present the main results as plots of false positive and negatives rates across representative conditions for exact, inexact, and near exact replications. Each case has two figures, the first showing FPR and the second FNR. I mark the conventionally accepted thresholds for false positive and negative error rates (0.05 and 0.2, resp.) as known landmarks to help interpret the results. I do **not** claim that these are the right thresholds for replications.
...
For this ideal case, replication works exactly as intuition predicts. FPR is the significance level divided by 2 (the factor of 2 because the effect sizes must have the same direction). Theory tell us that $FNR=1-power$ and though not obvious from the graph, the simulated data agrees rather well. 
