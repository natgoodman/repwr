...
- *d1.c2* (resp. *d2.c1*, *d1.p2*, *d2.p1*) - the standardized observed effect size (aka Cohen's d) of the one study is in the confidence interval (resp. prediction interval) of the other study
- *sdir* - the observed effects sizes for the two studies are in the same direction, i.e., have the same sign

Several measures depends on nuances of the *noncentral t-distribution*. The usual treatment parameterizes this distribution by *degrees of freedom* and a *noncentrality parameter*, and operates on the *t-statistic*. I prefer to work with more concrete notions: *sample size* (instead of degrees of freedom), *(presumed) population effect size* (instead of the noncentrality parameter), and *standardized observed effect size* aka *Cohen’s d* (instead of the t-statistic). For the limited context of my simulation, it's easy to convert between the concepts. I developed software that repackages base R's t-distribution functions to use my preferred concepts; I call this the *noncentral d2t-distribution*. To be clear: the d2t-distribution is not a new distribution in any sense; it's merely a repackaging of the t-distribution. I describe the d2t-distribution software in [Noncentral d2t-Distribution Applied to Measures of Replication Success](https://natgoodman.github.io/repwr/stats.stable.html). The d2t software does much of the heavy lifting for the measures.
...
The correctness criteria relevant to this document are 
...
The software implements a number of related correctness criteria, which I've not purused in detail: *nonz1or2*, either population effect size is non-zero; *nonz1and2*, both population effect sizes are non-zero; *nonz2*, the second population effect size is non-zero; *uni*, the first population effect size is non-zero and the two population effect sizes are equal. The software also allows a modest extension that allows the effect sizes to differ from 0 or each other by a specified *tolerance* $\epsilon$.
...
- *d1*, *d2*, *dm* mean the standardized observed effect size (aka *Cohen’s d*) of *s1*, *s2*, *sm*
...
- *scp1*, *scp2* are Uri Simonsohn's small telescopes thresholds of *s1*, *s2*
...
**Caution** The notation *d* is potentially ambiguous.  When used in measures it means $d_{sdz}$, the standardized observed effect size (aka Cohen’s $d$). When used in figures it generally means $d_{pop}$, the population effect size. Sorry.
...
| d1.c2 | *d1* contained in *c2* | 
| d2.c1 | *d2* contained in *c1* | 
| d1.cm | *d1* contained in *cm* | 
| d2.cm | *d2* contained in *cm* | 
| dm.c1 | *dm* contained in *c1* | 
| dm.c2 | *dm* contained in *c2* | 
| c1.c2 | *c1* and *c2* overlap | 
| c1.cm | *c1* and *cm* overlap |
| c2.cm | *c2* and *cm* overlap | 
| d1.p2 | *d1* contained in *p2* | 
| d2.p1 | *d2* contained in *p1* | 
| dm.p1 | *dm* contained in *p1* | 
| dm.p2 | *dm* contained in *p2* | 
...
Performance is critical, because I apply the measures to many, many replications (more than 100 million in default full runs). The code computes precursor statistics that make the measures very fast. It computes each precursor statistic as early as possible in the pipeline: it computes what it can before doing any simulations, then what it can on each simulated study individually, then statistics that depend only on the replication parameters ($n1$, $n2$, $d1_{pop}$, $d2_{pop}$), and finally statistics that depend on the actual replications. Once each replication set is formed and the precursor statistics computed, the software implements the measures as vectorized in-line code (i.e., not as functions) that operates on the entire set in a single gulp. 

### Statistics computed before simulation

Statistics computed before the simulation cannot depend on the data, of course. In addition to values that truly do not depend on the data (notably Uri Simonsohn's small telescope thresholds), I pre-compute expensive statistics on a parameter grid, then later interpolate to get the values for specific data. For interpolation, I use `aspline` from the [akima package](https://cran.r-project.org/web/packages/akima).
...
Uri's method first computes the smallest population effect size that would confer 33% power to the original study for seeing the smallest possible significant effect; Uri calls this $d_{33\%}$. Then it computes the smallest effect size the replica study could see consistent with the hypothesis that the true effect size is $\ge d_{33\%}$. I call this $d_{close}$. If the replica observes an effect size $\ge d_{close}$, the method deems it close enough to the original effect size to be valid; conversely, if the replica effect size is $\lt d_{close}$, the method deems it too small to have been really detected by the original study.

The extended effect size dependent method differs only in the first step: it computes the smallest population effect size that would confer adequate power (default 33% as in Uri's method) for the original study to see the *original* effect size rather than the *smallest possible* significant effect size. 
...
| $d_{sig}$ | `d.sig` | minimum significant effect size for a sample size of $n1$; for Uri's version of the method |
| $d1_{sdz}$ | `d1.sdz` | observed effect size in the original study; for effect size dependent version of the method |
...
These functions use the identities $d=t\sqrt{2n/n^2}$ and $t=d\sqrt{n^2/2n}$ to convert between standardized effect sizes ($d$) and t-statistics ($t$)  and base R's `pt` functions to convert t-statistics into p-values. 
...
d_{scope}&=d0 \mid p_d2t(n1,d_{sig},d0)=pwr \\
d_{close}&=d \mid p_d2t(n2,d,d_{scope})=\alpha \\
\bf{\text{effect size dependent method}}& \\
d_{scope}&=d0 \mid p_d2t(n1,d1_{sdz},d0)=pwr \\
d_{close}&=d \mid p_d2t(n2,d,d_{scope})=\alpha \\
\end{aligned}$$
...
I show here the code that implements the core computations and not the outer loop that invokes the core on all `n1`, `n2` pairs or `n1`, `n2`, `d1.dz` triples. The code uses R's `uniroot` function to solve the equations for $d_{scope}$ and $d_{close}$. It wraps `uniroot` in `suppressWarnings` to shut up an annoying warning from `pt` about possibly not attaining full precision. 
...
The software stores the results of Uri's method in a matrix `scope` whose row and column names are the values of `n1`, `n2` and values are `d.close`. It stores the effect size dependent results in a data frame `scopd` whose columns are `n1`, `n2`, `d1.sdz`, and `d.close`.

#### Confidence intervals

Confidence intervals depend on the sample size and observed effect size of an individual study. The software pre-computes confidence intervals on a grid of these values. The main work is done by the `ci_d2t` function described in the [d2t document](https://natgoodman.github.io/repwr/stats.stable.html). The result is a data frame `confvl` whose columns are `n`, `d.sdz`, 'd.lo`, `d.hi`. 
...
| ------- | --------------------------------------------------------------- |
| `n` | sample size |
| `d.sdz` | standardized observed effect size, aka *Cohen’s d* |
| `d.lo`, `d.hi` | bounds of confidence interval |

#### Prediction intervals

Prediction intervals depend on the sample sizes of the original and replica studies and the observed effect size of the original study. The software pre-computes prediction intervals on a grid of these values. The main work is done by the `pi_d2t` function described in the [d2t document](https://natgoodman.github.io/repwr/stats.stable.html). The result is a data frame `predvl` whose columns are `n1`, `n2`, `d1.sdz`, 'd.lo`, `d.hi`. 
...
| `d.sdz` | standardized effect size, aka *Cohen’s d* | `d.raw/sd` |
...
The code computes the inverse variance weight and weighted effect size for fixed-effect meta-analysis on each study, then combines the values to yield the final meta-analytic statistics for pairs of studies. 

The variance in question is the sampling variance of the estimated effect size ($d_{sdz}$), not the variance of the data! The sampling distribution of $d_{sdz}$ is a noncentral t-distribution centered on $d_{sdz}$ with $2(n-1)$ degrees of freedom. The weighted effect size is simply $d_{sdz}$ times the weight. I use the `sd\_d2t` function in the [d2t document](https://natgoodman.github.io/repwr/stats.stable.html) to get the standard deviation and square it to get the variance.
