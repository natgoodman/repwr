...
[Uri Simonsohn's small telescope](http://datacolada.org/wp-content/uploads/2015/05/Small-Telescopes-Published.pdf) thresholds depend only on the sample sizes of the original and replica studies. In Uri's words, the method "evaluates the original study’s *design* rather than its result — the telescope rather than the planet".

The method first computes the smallest population effect size that would confer 33% power to the original study; Uri calls this $d_{33\%}$. Then it computes the smallest observed effect size that the replica study could see consistent with the hypothesis that the true effect size is $\ge d_{33\%}$. I call this $d_{close}$. If the replica observes an effect size $\ge d_{close}$, the method deems it close enough to the original effect size to be valid; on the other hand, if the replica effect size is $\lt d_{close}$, the method deems it too small to have been really detected by the original study.
...
| $d_{sig}$ | `d.sig` | minimum significant effect size for a sample size of $n1$ |
...
The software performs the above computation on all pairs of simulated sample sizes (`n1`, `n2`). I show here the code that implements the computation. 
The code uses R's `uniroot` function to solve the equations for $d_{scope}$ and $d_{close}$. It wraps `uniroot` in `suppressWarnings` to shut up an annoying warning from `pt` about possibly not attaining full precision. I don't show the outer loop that invokes the core on all `n1`, `n2` pairs. 
