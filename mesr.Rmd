...
**sample size to degrees of freedom**
$$\begin{aligned}
df&=2(n-1) \\
\end{aligned}$$

**t-statistic to Cohen's d and p-value**. Note: $pt()$ is from base R.
$$\begin{aligned}
d_{sdz}&=t \sqrt{(2n)/n^2} \\ 
pval&=2 \times pt(-\lvert t\rvert,df=2*(n-1)) \\
\end{aligned}$$

**Cohen's d to t-statistic, p-value, and noncentrality parameter**. Note: $pt()$ is from base R.
$$\begin{aligned}
...
**p-value to t-statistic and Cohen's d**. Note: $qt()$ is from base R. $q\_d2t()$ is *d2t* wrapper for $qt$
$$\begin{aligned}
