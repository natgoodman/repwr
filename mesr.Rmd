...
Performance is critical, because I apply the measures to a large number of replications (more than 100 million in default full runs). The code computes whatever it can on individual studies and further pre-computes values that depend only the replication parameters ($n1$, $n2$, $d1{pop}$, $d2_{pop}$).
...
Once each replication set is formed, the software implements the measures as vectorized in-line code (i.e., not as a function) that operates on the entire set in a single gulp. 

>>> perhaps include some code here <<<

| measure | meaning | notes |
| ------- | ------- | ----- |
| sig1 | p-value of *s1* significant | precomputed on *s1* |
| sig2 | p-value of *s2* significant | precomputed on *s2* |
| sigm | p-value of *sm* significant | see below |
| sdir | $d1_{sdz}$ and $d2_{sdz}$ are in same direction, i.e., have same sign | `s1$d.sign==s2$d.sign`
