...
In the usual treatment, the central t-distribution is parameterized by *degrees of freedom*; the noncentral t-distribution also has a *noncentrality parameter*; and both operate on the *t-statistic*. I'm sure there are good reasons for these mysterious concepts, but I prefer the more concrete *sample size* and *standardized observed effect size* (aka *Cohen’s d*). For the limited context of this simulation, it's easy to convert between the concepts. While I'm at it, I define conversions between p-values and both my prefered units and t-statistics.
...
**t-statistic to Cohen's d and pval**
$$\begin{aligned}
d_{sdz}&=t \sqrt{(2n)/n^2} \\ 
pval&=2 \times pt(-\lvert t\rvert,df=2*(n-1)) \\
\end{aligned}$$

**Cohen's d to t statistic, pval, and noncentrality parameter**
$$\begin{aligned}
...
**pval to t statistic and Cohen's d**. Note: $qt()$ is from base R. $q_d2t()$ is *d2t* wrapper for $qt$
$$\begin{aligned}
t&=qt(pval/2,df=2*(n-1),lower.tail=F) \\
d_{sdz}&=q_d2t(n,q=pval/2,lower.tail=F) \\
\end{aligned}$$

I use these transformations to define a family of *d2t* functions analogous to R's *t* functions, plus a few more that I needed. These are in the file `R/stats.R`. In the code,

  - sample size (of each group): `n`
  - standardized observed effect size: `d`
  - center of noncentral t-distribution in `d` units: `d0`
  - p-value: `pval`
...
The final group compute the mean and standard deviation of the d2t-distribution. I was surprised to learn that base R doesn't provide these functions for the standard t-distribution. 
...
Performance is critical, because I apply the measures to many, many replications (more than 100 million in default full runs). The code computes whatever it can on individual studies and further pre-computes values that depend only the replication parameters ($n1$, $n2$, $d1_{pop}$, $d2_{pop}$).  Once each replication set is formed, the software implements the measures as vectorized in-line code (i.e., not as functions) that operates on the entire set in a single gulp. 

I now turn to specific measures and statistics used by measures.

### Measures and statistics computed on individual studies
...
Math: Computed from *n*, *d.sdz* by the standard formulae
$$\begin{aligned}
t&=d_{sdz} \times \sqrt{n^2/2n} \\
pval&=2 \times pt(-abs(t),df=2(n-1))
\end{aligned}$$

R:
``` {r eval=FALSE}
## Cohen's d to t statistic
d2t=function(n,d.sdz) d.sdz*sqrt(n^2/(2*n))
## t statistic to p-value
t2pval=function(n,t) 2*pt(-abs(t),df=2*(n-1))
## Cohen's d to p-value
d2pval=function(n,d.sdz) t2pval(n,d2t(n,d.sdz))
```

####  Pre-computed values for fixed-effect meta-analysis

Notation: $w_{meta}$ (`w.meta`), $d_{meta}$ (`d.meta`). Inverse variance weight and weighted effect size of an individual study. 

Definition: The variance in question is the sampling variance of the estimated effect size ($d_{sdz}$), not the variance of the data! The sampling distribution of $d_{sdz}$ is a noncentral t-distribution centered on $d_{sdz}$ with $2(n-1)$ degrees of freedom. The weighted effect size is simply $d_{sdz}$ divided by the weight.

Math: In the usual treatment, the noncentral t-distribution is defined in terms of *degrees of freedom* and *noncentrality parameter*. For the limited context of this simulation, the former is simply $2(n-1)$, and the latter $d_{sdz} \times \sqrt{n/2}$. The standard formula for variance of the noncentral t-distribution produces results in units of $t^2$. I translate these back to $d^2$ units using $d_{sdz}=t \times \sqrt{(2*n)/n^2}$. With these transformations in mind, the formula for variance is
$$\begin{aligned}
df&=2(n-1) \\
ncp&=d_{sdz} \times \sqrt{n/2} \\
\mu&=\sqrt{df/2} \times ncp \times \Gamma{(df-1)/2}/\Gamma{df/2} \\
var_{t}&=(1+ncp^2) \times df/(df-2)-\mu^2 \\
var&=var_{t} \times 2/n
\end{aligned}$$

R: The code breaks the problem into modular functions. It also deals with known limitations of R's `gamma` function
``` {r eval=FALSE}

```

These values are computed on individual studies, then combined to yield meta-effect size, etc.

``` {r eval=FALSE}
## inverse variance weight
w.meta=with(simr,1/sd_d2t(n,d.sdz)^2);
d.meta=with(simr,d.sdz*w.meta)
```
...
>>> meta-analysis
  - *dm* 
  - *dm* (standardized observed effect size for fixed-effect meta-analysis): 
