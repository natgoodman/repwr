...
- *pval1*, *pval2*, *pvalm* mean the p-value of *s1*, *s2*, *sm*
...
>>> mention notational ambiguity of *d* - in measures it means $d_{sdz}$, in figures it generally means $d_{pop}$
...
TODO maybe: explain which measures we use in the analysis, for plots and heats. explain that we look separately at scopes since they assume *sig2*
...
### Implementation notes for specific statistics

  - *d1*, *d2* (standardized observed effect size, aka *Cohen’s d*, for the two studies): mean of *group1* minus the mean of *group0* divided by the pooled standard deviation of the two groups.<br/>
The standard formula for pooled standard deviation when the two groups have the same sample size $n$ and standard deviations $sd0$ and $sd1$ is $\sqrt{((n-1)sd0^2+(n-1)sd1^2)/(2n -2))}$. This simplifies to $\sqrt{(sd0^2+sd1^2)/2}$ which is the formula I use. Note that this formula is independent of $n$.
