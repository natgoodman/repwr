...
Figure S1-1a shows FPR for $n1=20$ and $n2$ varying from 50 to 500. It's the same as Figure 1 in the blog post with the addition of a dashed red horizontal line at $sig.level/2=0.025$ (just barely visible beneath the solid red line depicting FPR). The x-axis shows all four parameters using $d1$, $d2$ as shorthand for $d1_{pop}$, $d2_{pop}$. $d1_{pop}=d2_{pop}=0$ throughout because this is the only way to get false positives with exact replications. Figure S1-2a shows FPR for *all* values of $n1$ using different colored lines for each value of $n1$; the lines fall on top of each other illustrating that $n1$, the original sample size, has no effect on the results. I checked the numbers driving the graphs to make sure I hadn't inadvertently used the same data for all values of $n2$; the data are similar but not exactly the same.

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S01001a_exact_fpr_n1=020.png');
knitr::include_graphics('figure/resig/m=1e4/figure_S01001a_exact_fpr_by_n1.png');
```

Figures S1 and S2 are essentially identical illustrating that $n1$, the original sample size, has no effect on the results. I checked the numbers driving the graphs to make sure I hadn't inadvertently used the same data for both cases; the data are similar but not exactly the same.
