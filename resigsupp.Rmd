...
I systematically explore the parameter space rather then selecting results of greatest interest to blog readers. 

I display the results using a few kinds of graphs handcrafted for my purposes. 

Four parameters define each pairwise replication: $n1$, $n2$, $d1_{pop}$, and $d2_{pop}$. These are the sample and population effect sizes for the two studies.

>>> more TBD
1. line graphs of rates: simple and intuitive, I think, but not good at showing data that varies across many parameters
...
A replication is *exact* if the two studies are sampling the same population; this means $d1_{pop}=d2_{pop}$. For this ideal case, replication works exactly as intuition predicts. As we'll see, the simulated error rates match theory quite well.
...
Figure S2-1 is like Figure 2 in the blog post with the addition of dashed lines depicting $1-power2$ and with the legend moved down a smidge. It shows FNR for $n1=20$ and $n2$ varying from 50 to 500 (the same values as in Figure S1) but with $d1_{pop}=d2_{pop}$ ranging from 0.1 to 1. Figure S4 is the same but with $n1=200$. 
