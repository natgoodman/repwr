...
This document contains supplementary material for the blog post [Systematic Replication May Make Many Mistakes](https://replicationnetwork.com/2018/09/28/goodman-systematic-replication-may-make-many-mistakes/) kindly posted by Bob Reed on The Replication Network.

## Results

### Exact replications

A replication is *exact* if the two studies are sampling the same population; this means $d1_{pop}=d2_{pop}$. For this ideal case, replication works exactly as intuition predicts. Theory tell us that FPR is the significance level divided by 2 (the factor of 2 because the effect sizes must have the same direction) and $FNR=1-power2$, where $power2$ is the power of the replica study. As we'll see, the simulated FPR is dead-on, but the simulated FNR is not as good, esp. when $n1$ and $d$ are small.

Figures S1 is the same as Figure 1 in the blog post with the addition of a dashed red horizontal line at $sig.level/2=0.025$ (just barely visible beneath the solid red line depicting FPR). The figure shows FPR for $n1=20$ and $n2$ varying from 50 to 500. The x-axis shows all four parameters using $d1$, $d2$ as shorthand for $d1_{pop}$, $d2_{pop}$. $d1_{pop}=d2_{pop}=0$ throughout because this is the only way to get false positives with exact replications. Figure S2 is the same but for $n1=200$, 10x the previous value. There is a barely visible dashed red line at 0.025 in both figures to demonstrate that $FPR=sig.level/2=0.025$ as theory predicts.  

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S001_exact_fpr_n1=020.png');
knitr::include_graphics('figure/resig/m=1e4/figure_S002_exact_fpr_n1=200.png');
```

Figures S1 and S2 are essentially identical illustrating that $n1$, the original sample size, has no effect on the results.

Figure S3 is like Figure 2 in the blog post with the addition of dashed lines depicting $1-power2$ and with the legend moved down a smidge. It shows FNR for $n1=20$ and $n2$ varying from 50 to 500 (the same values as in Figure S1) but with $d1_{pop}=d2_{pop}$ ranging from 0.1 to 1. Figure S4 is the same but with $n1=200$. 

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S003_exact_fnr_n1=020.png');
knitr::include_graphics('figure/resig/m=1e4/figure_S004_exact_fnr_n1=200.png');
```

For $n1=20$ (Figure S3), the simulated FNR is a bit greater than theory predicts when $d$ is small. For $n1=200$ (Figure S4), the fit is good for all values of $d$: the dashed lines are mostly hidden by the solid ones.

I get worried when my simulated results differ consistently from theory, even when the discrepancy is small.  So I explored.  It turns out the difference is due to the same-direction requirement; when power is low, same-direction has a large enough effect to increase FNR noticeably. Figures S5 and S6 repeat the figures with same-direction switched off. Theory and simulation now agree quite well for both values of $n1$.

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S005_exact_fnr_nosdir_n1=020.png');
knitr::include_graphics('figure/resig/m=1e4/figure_S006_exact_fnr_nosdir_n1=200.png');
```

Figures S7 and S8 plots FNR vs. $1-power2$ across the entire dataset with same-direction switch on (Figure S7) and off (Figure S8). The dots are color-coded by $power1$, the power of the original study (smaller power has lighter dots). The points fall nicely along the diagonal in both cases, with less scatter without same-direction, as expected.

The correlation of the two statistics across the enitre dataset is 0.9997737 and 0.9999019 resp. under the two conditions.  When limiting to small values of $d2$, $d\le0.2$, the correlations are 0.9989049 and 0.9995707.

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S007_exact_fnr_vs_power.png');
knitr::include_graphics('figure/resig/m=1e4/figure_S008_exact_fnr_vs_power_nosdir.png');
```

As one would expect and Figures S3-S6 illustrate, if the population effect size is small, $n2$ must be large to reliably yield a positive result. For $d=0.2$, $n2$ must be almost 400 in theory and 442 in the simulation to achieve $FNR=0.2$; to hit $FNR=0.05$, $n2$ must be more than 650 (in theory). 

Figure S9 shows the values of $n2$ and $d2$ that achieve FNR cutoffs of 0.05 and 0.20 for $n1=20$ and $n1=200$.

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/resig/m=1e4/figure_S009_exact_fnr_cutoff.png');
```
...
Figure 3 uses the same values of $n1$, $n2$, and $d1_{pop}$ as in Figure 1, but with $d2_{pop}$ ranging from 0 to 1. Figure 4 has the same parameters but fixes $d1_{pop}=0.5$, a medium effect size.

```{r out.width="50%",echo=FALSE,fig.show='asis'}
## knitr::include_graphics('figure/resig/m=1e4/figure_003_inexact_fpr.png');
## knitr::include_graphics('figure/resig/m=1e4/figure_004_inexact_fnr.png');
```

FPR is bad unless $d2_{pop}$ is small and gets worse as $n2$ (and $d2_{pop}$) increase. It may seem odd that the error rate increases as the sample size increases. What's going on is a consequence of power. More power is usually good, but in this setting every positive is a false positive, so more power is bad. 

FNR behaves in the opposite direction: bad when $n2$ is small and better as $n2$ (and $d2_{pop}$) increase. FNR also depends on $d1_{pop}$, getting worse when $d1_{pop}$ is smaller and better when $d1_{pop}$ is bigger (data not shown).
...
I don't know what value of $near$ is reasonable for a systematic replication project. I imagine it varies by research area depending on the technical difficulty of the experiments and the variability of the phenomenon. The range 0.1-0.3 feels reasonable. The figures extend the range by 0.1 on each end just to be safe.

Figures 5 and 6 uses the same parameters as Figures 3 and 4, resp., but include $near$ ranging from 0 to 0.4.  In these figures, $d2_{pop}$ ranges from $d1_{pop}-near$ to $d1_{pop}+near$ with values $<0$ discarded. As we saw in the inexact case, FPR worsens and FNR improves as $n2$ grows. 


```{r out.width="50%",echo=FALSE,fig.show='asis'}
## knitr::include_graphics('figure/resig/m=1e4/figure_005_nearexact_fpr.png');
## knitr::include_graphics('figure/resig/m=1e4/figure_006_nearexact_fnr.png');
```

To show the tradeoff between FPR and FNR, Figure 7 plots both error rates for $near=0.1$ and $near=0.3$. 

```{r out.width="50%",echo=FALSE,fig.show='asis'}
## knitr::include_graphics('figure/resig/m=1e4/figure_007_nearexact_fpr+fnr.png');
```
