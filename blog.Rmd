...
I simulate these rules (and more) across many replication conditions and compute false positive and false negative rates across a range of conditions. The simulations vary the samples size from 10 to 1000 and true effect size from 0 to 1; in total, I simulate <<12,100>> conditions each with $10^4$ simulated replications, for a grand total of more than <<100 million>> simulated instances.
...
1. *non-zero* - a replication instance is *true* if $d1_{pop}\ne0$, i.e., the population effect size for the first study is non-zero
2. *same-effect* - a replication instance is *true* with *tolerance* $\delta$ if $abs(d1_{pop}-d2_{pop})\le\delta$, i.e., the two population effect sizes differ by at most $\delta$
...
- a *false negative* occurs when $abs(d1_{pop}-d2_{pop})\le\delta$, but the measure returns false
...
The parameter space is vast, because it's possible to vary each of the four paramters ($n1$, $n2$, $d1{pop}$, $d2_{pop}$) across a considerable range. It's challenging to present the data in a comprehensive, yet concise and intelligible, manner.  I use several kinds of graphs.
...
Figures 1-4 show false positive rates for two values of $n1$ (20 and 40) and a range of values for $n2$. The smallest $n2$ in each case satisfies the recommendation in [Uri's post on the 90x75x50 heuristic](http://datacolada.org/54) that $n2=2.5 \times n1$ is big enough. The x-axis shows all four parameters using $d1$, $d2$ as shorthand for $d1_{pop}$, $d2_{pop}$ to conserve space. $d1_{pop}=d2_{pop}=0$ throughout because this is the only way to get false positives for *non-zero* with exact replications.

The first two figures are line graphs with the false positive rate along the y-axis; these graphs use color and other line properties to distinguish the measures. The latter two are heatmaps with measures along the y-axis; these plots use color (shades of red and blue) to depict the the false positive rate; the switch from red to blue is set at the conventionally accepted threshold of 0.05 for false positives. 

```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/blog/figure_001_nonzro_exact_fpr_n1=20_plot.png');
knitr::include_graphics('figure/blog/figure_002_nonzro_exact_fpr_n1=40_plot.png');
knitr::include_graphics('figure/blog/figure_003_nonzro_exact_fpr_n1=20_heat.png');
knitr::include_graphics('figure/blog/figure_004_nonzro_exact_fpr_n1=40_heat.png');
```
...
```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/blog/figure_005_nonzro_exact_fnr_n1=20_heat.png');
knitr::include_graphics('figure/blog/figure_006_nonzro_exact_fnr_n1=40_heat.png');
```
...
```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/blog/figure_007_nonzro_exact_roc.png');
```
...
```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/blog/figure_008_nonzro_inexact_roc.png');
```
...
The results are discouraging. It doesn't take much separation to exceed acceptable error rates. When $d1_{pop}$ and $d2_{pop}$ differ by as little as 0.05, abut half the points lie beyond the acceptable 0.05 false positive rate; by time we get to 0.2, all points are bad.
...
```{r out.width="50%",echo=FALSE,fig.show='asis'}
knitr::include_graphics('figure/blog/figure_015_small_telescopes_nonzro_roc.png');
knitr::include_graphics('figure/blog/figure_016_small_telescopes_nonzro_rag.png');
knitr::include_graphics('figure/blog/figure_017_small_telescopes_sameff_roc.png');
knitr::include_graphics('figure/blog/figure_018_small_telescopes_sameff_rag.png');
```
