...
| ------- | ------- | -------------------------------------------------------- |
| $n$ | `n` | sample size |
| $d_{pop}$ | `d.pop` | population effect size; this arises in discussion but not the software |
...
| $d0$ | `d0` | noncentrality parameter expressed in $d_{sdz}$ units; this is the presumed population effect size |
| $pval$ | `pval` | p-value |
...
The suffixes "1" and "2" denote the two studies in both math and code, e.g, $n1$ (`n1`), $n2$ (`n2`) are the sample sizes of the two studies. It is often convenient to think of study "1" as the original and "2" the replica but this isn't baked into the math or code.
...
I use the transformations above to define a family of d2t-distribution functions analogous to R's t-distribution functions, plus a few more. 
...
I was surprised to learn that base R doesn't provide mean and standard deviation functions for even the central t-distribution, so I had to dig it all out from references on the web, mostly **References TBD**. 
...
## Confidence Intervals of d2t-Distribution
