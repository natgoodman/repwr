...
| `conf.level`, `pred.level` | $c$ | confidence level for confidence or prediction interval; I use the same math symbol for both kinds of interval, since no ambiguity is possible | 
...
Note that $ci\_d2t()$ used in the math (`ci_dt` in the code) is the confidence interval function for the d2t-distribution defined above.


The code seems to work, but I have no intuition why.
